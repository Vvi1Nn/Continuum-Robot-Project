...
class Motor(CanOpenBusProcessor):
    ...
    @classmethod
    def config(cls, mode="position_control", acc=1000, dec=10000, vel=100) -> None:
    ...
    def __set_mode(self) -> None:
        if self.sdo_write_32("control_mode", Motor.control_mode):
            ...
            print("\033[0;31m[Motor {}] set control mode failed\033[0m".format(self.node_id))
    def __set_acceleration(self):
        if self.sdo_write_32("acceleration", Motor.acceleration):
            ...
            print("\033[0;31m[Motor {}] set acceleration failed\033[0m".format(self.node_id))   
    def __set_deceleration(self):
        if self.sdo_write_32("deceleration", Motor.acceleration):
            ...
            print("\033[0;31m[Motor {}] set deceleration failed\033[0m".format(self.node_id))
    def __set_velocity(self):
        if self.sdo_write_32("velocity", Motor.velocity):
            ...
            print("\033[0;31m[Motor {}] set velocity failed\033[0m".format(self.node_id))

    def __check_bus_status(self) -> bool:
        if super().check_bus_status():
            print("\033[0;32m[Motor {}] bus ready\033[0m")
    
    def get_motor_status(self) -> str:
        value = self.sdo_read("status_word")
    ...
    def __check_motor_status(self):
        self.get_bus_status()
        while self.motor_status != "switched_on":
            self.sdo_write_32("control_word", protocol.CONTROL_WORD["servo_ready/stop"])
            time.sleep(0.5)
        print("\033[0;32m[Motor {}] motor ready\033[0m".format(self.node_id))
    
    def __init_config(self):
        self.__check_bus_status()
        self.__check_motor_status()
