...
class ControlPanel(QMainWindow):
    def __init__(self, account) -> None:
        ...
        self.__usbcan_0 = UsbCan(channel="0")
        self.__usbcan_1 = UsbCan(channel="1")
    ...
    def enable_channel1(self, flag_open, text_open, flag_reset, text_reset):
    ...
    def enable_close_device(self, flag, text):
    ...
    def enable_set_param(self, flag, text0, text1, text2):
    ...
    def enable_init_motor(self, flag, text):
    ...
    def set_default_show(self) -> None:
        self.enable_menu(True, False, False) # menu 初始化激活 控制失效 日志失效
        
        self.ui.stackedWidget.setCurrentIndex(0)

        self.enable_open_device(True, "打开设备") # usbcan
        self.enable_channel0(False, "打开通道0", False, "重置通道0")
        self.enable_channel1(False, "打开通道1", False, "重置通道1")
        self.enable_close_device(False, "关闭设备")

        self.enable_default() # motor
    ...
    def set_usbcan_jumping(self) -> None:
        def open_usbcan():
            ...
            if UsbCan.is_open:
                ...
                self.enable_channel0(True, "打开通道0", False, "重置通道0")
                self.enable_channel1(True, "打开通道1", False, "重置通道1")
        def start_channel_0():
            self.__usbcan_0.timer = self.ui.bx_rate0.currentText()
            self.__usbcan_0.init_can()
            self.__usbcan_0.start_can()
            if self.__usbcan_0.is_start:
                self.enable_channel0(False, "通道0已打开", True, "重置通道0")
                if self.__is_admin:
                    self.enable_save_param(True) # 激活 保存
                    self.enable_set_param(True, "默认值1000", "默认值10000", "默认值100") # 激活输入
                else:
                    Motor.config(self.__usbcan_0) # 直接保存默认参数
                self.enable_init_motor(True, "生效") # 激活 生效
        def start_channel_1():
            self.__usbcan_1.timer = self.ui.bx_rate1.currentText()
            self.__usbcan_1.init_can()
            self.__usbcan_1.start_can()
            if self.__usbcan_1.is_start:
                self.enable_channel1(False, "通道1已打开", True, "重置通道1")
                pass # 其他的配置 后续补齐
        def reset_cannel_0():
    ...
    def __interface_init_motor(self):
        self.enable_set_param(False, "当前值{}".format(Motor.acceleration), "当前值{}".format(Motor.deceleration), "当前值{}".format(Motor.velocity))
        self.enable_save_param(False)
        ...
        self.enable_init_motor(False, "成功")
        self.enable_menu(True, True, False)
